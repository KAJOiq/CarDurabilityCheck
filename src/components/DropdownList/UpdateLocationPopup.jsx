...
const UpdateLocationPopup = ({ onClose, refreshData, locationId, locationName: initialLocationName }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetchData(
        `admin/lookup/update-traffic-location?Id=${locationId}&NewName=${encodeURIComponent(locationName)}`,
        {
          method: "PUT",
          headers: {
            "accept": "*/*",
      ...
      if (response.isSuccess) {
        setSuccess("تم تعديل الموقع بنجاح!");
        refreshData(); 
        setTimeout(() => {
          onClose(); 
        }, 1000);

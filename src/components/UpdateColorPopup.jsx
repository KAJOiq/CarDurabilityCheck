...
const UpdateColorPopup = ({ onClose, refreshData, colorId, color: initialColor }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetchData(
        `admin/lookup/update-color?Id=${colorId}&NewColor=${encodeURIComponent(color)}`,
        {
          method: "PUT",
          headers: {
            "accept": "*/*",
      ...
      if (response.isSuccess) {
        setSuccess("تم تعديل اللون بنجاح!");
        refreshData();
        setTimeout(() => {
          onClose(); 
        }, 1000);
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-lg flex items-center justify-center p-4" dir="rtl">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden">
        <div className="p-6">
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1">لون المركبة</label>
            <input
              type="text"
              value={color}
